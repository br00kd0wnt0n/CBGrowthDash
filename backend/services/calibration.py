        ...
        for sh in sheets:
            ...
            if sheetData is not None:
                for row in sheetData:
                    rows.append([get_text(c) for c in row])
            out[name] = rows
        return out
...
def load_follower_history_from_xlsx(xlsx_path: Path) -> Dict[str, Any]:
    ...
    Prefers the 'Growth Chart' tab with explicit per-month, per-platform values.
    Falls back to reconstructing from 'Historical Growth' if needed.
    """
    rows_by_sheet = _xlsx_read_rows(xlsx_path)
